%TO DO: все перевести и проверить грамматику

% * TO DO добавить список литературы для тех, кто интересуется, или кому мое изложение не понравится

%Мажуга
%TO DO Геометрический смысл для всего
%TO DO Основная теорема алгебры бд со ссылкой на док-во (возможно, нужно само доказательство)
...
%answer: а) $2^{n / 2} \cos \frac{n \pi}{4}$. Calculate $(1+i)^n$ using the binomial theorem and de Moivre's formula.
...
%answer: $\frac{1}{2}\left(2^{n-1}+2^{n / 2} \cos \frac{n \pi}{4}\right)$; use а) and $\sum_{k=0}^n\left(\begin{array}{l}n \\ k\end{array}\right)=2^n$; $\sum_{k=0}^n(-1)^k\left(\begin{array}{l}n \\ k\end{array}\right)=0$.
